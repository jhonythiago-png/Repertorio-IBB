...
def sync():
    ...
    for title, song_data in aggregated_songs.items():
        ...
        # Se o título, o artista ou a combinação estiver na lista negra, pula
        if title.lower() in ignored_titles or artist.lower() in ignored_titles or full_check in ignored_titles:
            print(f"Pulo: '{title}' ou '{artist}' está na lista de exclusão.")
            continue
