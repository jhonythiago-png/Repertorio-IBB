...
def get_youtube_id(query):
    """Tenta buscar o ID real do vídeo no YouTube via busca simples."""
    try:
        # Codifica a busca
        search_url = f"https://www.youtube.com/results?search_query={query.replace(' ', '+')}"
        response = requests.get(search_url, timeout=5)
        # O ID do vídeo aparece no HTML do YouTube nesse formato
...
def sync():
    ...
    # Carrega cache de IDs (para não buscar toda hora o mesmo vídeo)
    cache = {}
    ...
    for name, songs in all_categories.items():
        for song in songs:
            if song["title"] in overrides:
                ...
                # Tenta extrair ID do link manual para a capa
                id_m = re.search(r"(?:v=|\/embed\/|youtu\.be\/|\/v\/)([^&?#/ ]+)", song["url"])
                if id_m: song["vid_id"] = id_m.group(1)
            else:
                # Se já temos no cache, usa. Se não, busca no YouTube.
                vid_id = cache.get(song["title"])
                if not vid_id:
                    # vid_id = get_youtube_id(song["query"]) # Removemos o print direto para evitar erro de encoding no terminal
