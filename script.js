...
window.addEventListener('DOMContentLoaded', () => {
    ...
    function _fillScalingMenu() {
        ...
        if (song && song.cifra_text) {
            ...
            if (tomLineMatch) {
                ...
                if (forma) {
                    ...
                    if (fI !== -1) {
                        // Se tem capo, o Tom Real = Forma + Capo
                        detKey = SCALE[(fI + detCapo) % 12];
                    } else {
                        detKey = fK;
                    }
                }
        ...
        SCALE.forEach(note => {
            ...
            if (normalizeKey(note) === normalizeKey(detKey)) opt.selected = true;
            tomSelect.appendChild(opt);
        });
