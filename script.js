    ...
    try {
        ...
        data.forEach((setItem, idx) => {
            ...
            card.onclick = (e) => {
                ...
                if (e.target.closest('.setlist-card-remove-btn')) return;
                const songToOpen = originalSong || { title: setItem.song_title, artist: 'Desconhecido', url: '', cifra_text: '' };
                _setlistIndex = _setlistSongs.findIndex(s => s.title === songToOpen.title);
    ...
    if (addToSetlistBtn) {
        addToSetlistBtn.onclick = () => {
            ...
            if (!isVisible) {
                ...
                const song = window._modalCurrentSong;
                const _SCALE = ['C', 'Db', 'D', 'Eb', 'E', 'F', 'F#', 'G', 'Ab', 'A', 'Bb', 'B'];
                    ...
                    if (tomRx) {
                        ...
                        for (const line of txt.split('\n')) {
                            ...
                            if (allChords) {
                                ...
                                const idx  = _SCALE.findIndex(n => n.toLowerCase() === base.toLowerCase() || (base === 'A#' && n === 'Bb') || (base === 'Db' && n === 'C#'));
                                const realIdx = idx !== -1 ? idx : _SCALE.indexOf(base);
                                if (realIdx !== -1) {
                                    detectedKey = detectedCapo > 0
                                        ? _SCALE[(realIdx + detectedCapo) % 12]
                                        : base;
                                }
                                if (passedIntro) break;
                            }
                ...
                // Garante que detectedKey é válido
                if (!_SCALE.includes(detectedKey)) detectedKey = 'C';
