        ...
        addToSetlistBtn.onclick = () => {
            try {
                ...
                if (!isVisible) {
                    ...
                    // 1) POPULAÇÃO BÁSICA GARANTIDA (Impossível ficar vazio)
                    tomSelect.innerHTML = '';
                    const basicNotes = ['C', 'Db', 'D', 'Eb', 'E', 'F', 'F#', 'G', 'Ab', 'A', 'Bb', 'B'];
                    
                    // Valores para detecção
                    let detectedKey = 'C';
                    let detectedQuality = '';
                    let detectedCapo = 0;

                    // 2) Tenta detectar se houver música aberta
                    if (song && song.cifra_text) {
                        const txt = song.cifra_text;
                        
                        // Capo (Regex simplificado sem caracteres especiais)
                        ...
                        if (tomRx) {
                            ...
                            if (forma) {
                                ...
                                const fIdx = basicNotes.indexOf(fKey);
                                if (fIdx !== -1) {
                                    detectedKey = basicNotes[(fIdx + detectedCapo) % 12];
                                }
                            }
                        }
                    }

                    // 3) Cria as opções no Select
                    basicNotes.forEach(note => {
                        const displayNote = note + detectedQuality;
                        const opt = document.createElement('option');
                        opt.value = displayNote;
                        opt.textContent = displayNote;
                        // Compara normalizado para marcar o correto
                        if (normalizeKey(note) === normalizeKey(detectedKey)) opt.selected = true;
                        tomSelect.appendChild(opt);
                    });
