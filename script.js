        ...
        addToSetlistBtn.onclick = () => {
            ...
            if (!isVisible) {
                ...
                if (song && song.cifra_text) {
                    ...
                    let detectedQuality = '';
                    if (tomRx) {
                        const fullKey = tomRx[1];
                        if (fullKey.toLowerCase().endsWith('m')) {
                            detectedQuality = 'm';
                            detectedKey = normalizeKey(fullKey.replace(/m$/i, ''));
                        } else {
                            detectedKey = normalizeKey(fullKey);
                        }
                    } else {
                        let passedIntro = false;
                        for (const line of txt.split('\n')) {
                            ...
                            const allChords = tokens.length > 0 && tokens.every(tk =>
                                /^[A-G][b#]?(?:m(?:aj|in)?|maj|dim|aug|sus|add|min|°|ø|º|-)?-?(?:\d{1,2})?(?:M)?(?:[+])?(?:\((?:[b#]?\d+[+\-]?(?:\/[b#]?\d+[+\-]?)*)\))?(?:\/[A-G][b#]?(?:\d{0,2})?)?$/.test(tk)
                            );
                            if (allChords) {
                                const base = tokens[0].split('/')[0];
                                const match = base.match(/^([A-G][b#]?m?)/i);
                                const rootWithQuality = match ? match[1] : base;
                                if (rootWithQuality.toLowerCase().endsWith('m')) {
                                    detectedQuality = 'm';
                                }
                                const rootOnly = rootWithQuality.replace(/m$/i, '');
                                const idx = SCALE.indexOf(normalizeKey(rootOnly));
                                if (idx !== -1) {
                                    detectedKey = SCALE[(idx + detectedCapo) % 12];
                                }
                                if (passedIntro) break;
                            }
                        }
