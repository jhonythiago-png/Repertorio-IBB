...
window.addEventListener('DOMContentLoaded', () => {
    ...
    function _fillScalingMenu() {
        ...
        if (song && song.cifra_text) {
            ...
            const tM = txt.match(/^Tom:\s*([A-G][b#]?m?)/im);
            if (tM) {
                const full = tM[1];
                detQual = full.toLowerCase().endsWith('m') ? 'm' : '';
                detKey = normalizeKey(full.replace(/m$/i, ''));
            } else {
                const forma = typeof detectFormaKey === 'function' ? detectFormaKey(txt) : null;
                if (forma) {
                    detQual = forma.toLowerCase().endsWith('m') ? 'm' : '';
                    const fK = normalizeKey(forma.replace(/m$/i, ''));
                    const fI = SCALE.indexOf(fK);
                    if (fI !== -1) detKey = SCALE[(fI + detCapo) % 12];
                }
            }
        }

        // 2) Popula o Select (Maior e Menor)
        tomSelect.innerHTML = '';
        SCALE.forEach(note => {
            // Opção Maior
            const oMaj = document.createElement('option');
            oMaj.value = note; oMaj.textContent = note;
            if (normalizeKey(note) === normalizeKey(detKey) && detQual === '') oMaj.selected = true;
            tomSelect.appendChild(oMaj);

            // Opção Menor
            const oMin = document.createElement('option');
            oMin.value = note + 'm'; oMin.textContent = note + 'm';
            if (normalizeKey(note) === normalizeKey(detKey) && detQual === 'm') oMin.selected = true;
            tomSelect.appendChild(oMin);
        });
