...
function openVideo(song) {
    ...
    document.getElementById('confirmDeleteBtn').onclick = async () => {
        ...
        console.log('Tentando excluir música:', { id: song.id, title: song.title });

        try {
            // Tenta deletar por ID primeiro (mais seguro), se não tiver ID usa o título
            // Adicionamos .select() para retornar as linhas deletadas e confirmar o sucesso real
            const query = song.id ? _supabase.from('songs').delete().eq('id', song.id).select()
                                 : _supabase.from('songs').delete().eq('title', song.title).select();
            ...
            if (!data || data.length === 0) {
                console.warn('Nenhuma linha foi deletada. Verfique se a música ainda existe ou se há permissão.');
                alert(`⚠️ A música "${song.title}" não pôde ser encontrada ou já foi removida.`);
            } else {
                alert(`✅ "${song.title}" foi excluído com sucesso do repertório.`);
            }

            modal.style.display = 'none';
            videoIframe.src = '';
            
            // Pequeno delay para garantir que o BD processou antes de recarregar
            setTimeout(async () => {
                await fetchData();
            }, 300);
...
searchInput.oninput = (e) => {
    ...
    allSongs.forEach(s => {
        ...
        if (!uniqueSongsMap.has(cleanTitle)) {
            uniqueSongsMap.set(cleanTitle, s);
        }
    });
