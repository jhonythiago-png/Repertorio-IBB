    ...
    if (addToSetlistBtn) {
        addToSetlistBtn.onclick = () => {
            try {
                ...
                // Correção: Verifica se está visível de forma robusta
                const isVisible = panel.style.display === 'block';
                panel.style.display = isVisible ? 'none' : 'block';
                document.getElementById('urlEditorPanel').style.display = 'none';

                if (isVisible) return; // Se estava fechando, encerra aqui

                const modalObsEl = document.getElementById('modalObs');
                const obsModal = modalObsEl ? modalObsEl.textContent.replace('Obs: ', '').trim() : '';
                const obsInput = document.getElementById('setlistObsInput');
                if (obsInput) obsInput.value = obsModal;
                ...
                if (song && song.cifra_text) {
                    ...
                    const capoMatch = txt.match(/Capo\s*(\d+)/i) || txt.match(/Capotraste\s+na\s+(\d+)/i);
                    if (capoMatch) detectedCapo = parseInt(capoMatch[1]);

                    const tomRx = txt.match(/^Tom:\s*([A-G][b#]?m?)/im);
                    if (tomRx) {
                        const full = tomRx[1];
                        detectedQuality = full.toLowerCase().endsWith('m') ? 'm' : '';
                        detectedKey = normalizeKey(full.replace(/m$/i, ''));
                    } else {
                        const forma = typeof detectFormaKey === 'function' ? detectFormaKey(txt) : null;
                        if (forma) {
                            detectedQuality = forma.toLowerCase().endsWith('m') ? 'm' : '';
                            const fKey = normalizeKey(forma.replace(/m$/i, ''));
                            const fIdx = SCALE.indexOf(fKey);
                            if (fIdx !== -1) detectedKey = SCALE[(fIdx + detectedCapo) % 12];
                        }
                    }
                }

                // 2) POPULAÇÃO DO SELECT (Listando Maior e Menor separadamente)
                tomSelect.innerHTML = '';
                SCALE.forEach(note => {
                    // Opção Maior
                    const optMaj = document.createElement('option');
                    optMaj.value = note; optMaj.textContent = note;
                    if (normalizeKey(note) === normalizeKey(detectedKey) && detectedQuality === '') optMaj.selected = true;
                    tomSelect.appendChild(optMaj);

                    // Opção Menor
                    const optMin = document.createElement('option');
                    optMin.value = note + 'm'; optMin.textContent = note + 'm';
                    if (normalizeKey(note) === normalizeKey(detectedKey) && detectedQuality === 'm') optMin.selected = true;
                    tomSelect.appendChild(optMin);
                });

                if (capoSelect) capoSelect.value = String(detectedCapo);

            } catch (err) {
                console.error('Erro no menu de escala:', err);
            }
        };
    }
