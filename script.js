    ...
    if (addToSetlistBtn) {
        addToSetlistBtn.onclick = () => {
            ...
            if (!isVisible) {
                ...
                // 1) Valores padrão (Dó maior e sem capo)
                let detectedKey     = 'C';
                let detectedQuality = '';
                let detectedCapo    = 0;
                ...
                if (song && song.cifra_text) {
                    ...
                    // Detecta Capo
                    const capoMatch = txt.match(/Capotraste\s+na\s+(\d+)[aª°]?\s*casa/i) || txt.match(/Capo\s*(\d+)/i);
                    if (capoMatch) detectedCapo = parseInt(capoMatch[1]);

                    // Detecta Tom pela linha "Tom: ..."
                    ...
                    if (tomRx) {
                        ...
                        // Se não tem linha "Tom:", tenta detectar pelos acordes (Forma)
                        const forma = detectFormaKey(txt);
                        if (forma) {
                            ...
                            if (fIdx !== -1) {
                                // Tom Real = Forma + Capo
                                detectedKey = SCALE[(fIdx + detectedCapo) % 12];
                            }
                        }
                    }
                }

                // 2) Popula o Select de Tom (Garantindo que nunca fique vazio)
                tomSelect.innerHTML = '';
                SCALE.forEach(note => {
                    ...
                    // Compara a nota normalizada para garantir o 'selected' (ex: Gb vs F#)
